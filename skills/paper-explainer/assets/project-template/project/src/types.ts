...
export interface SceneStep {
  ...
  camera: { frameId: string | null; requestReason: CameraRequestReason | null; targetIds: string[]; mode: "fit" | "tight" | "contextual"; bounds: Bounds };
...
export interface Scene { id: string; title: string; eyebrow?: string; contentKind: string; worldId: string; claimIds: string[]; evidenceIds: string[]; presentation?: ScenePresentation; steps: SceneStep[] }
export interface SceneIR { schemaVersion: "2.0" | "2.1"; paperId: string; title?: string; build: { compilerVersion: string; templateCatalogVersion: string; sourceHash: string }; worlds: CompiledWorld[]; scenes: Scene[] }
