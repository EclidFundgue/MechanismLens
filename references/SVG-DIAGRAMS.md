...
论文图表有两条合法路径，按「怎样讲得更准、更清楚」选择：

1. **SVG 重绘（首选）**——结构清晰的架构图 / 流程图 / 结果图表。好处：
   准确、可动效、可逐步揭示、随主题换色。
2. **原图嵌入**——复杂大图、定性结果（照片 / 渲染 / 视频帧）、密集
   表格、公式。从 PDF 或 arXiv LaTeX 源取出，裁切干净、注明编号；
   原图同样要由 `step` 驱动（高亮框 / 局部放大 / 逐块揭示）。
   取图与嵌入规范见 `PAPER-ASSETS.md`。

**禁止的只有一种**：用图像生成模型画架构图 / 流程图 / 结果图（会画错
结构）。**不是**禁止贴论文原图。

> 封面 / 概念插画 / 氛围图可用 `gpt-image-2`（见 `PAPER-ASSETS.md` §0）。
...
| 内容 | 选择 |
|---|---|
| 模块 ≤ 10 个、连接关系清晰 | SVG 重绘 |
| 模块很多 / 连线复杂 / 有大量细节 | 原图 + 高亮框 + 局部放大 |
| 结果表格（≤ 2 行 × 6 列） | SVG / flex 重绘 |
| 结果表格（大而密） | 原图裁切 + 行高亮 |
| 定性结果（照片 / 渲染 / 对比图） | **原图**（SVG 无法忠实复现） |
| 公式 | KaTeX（推荐）或原图（见 `PAPER-ASSETS.md` §3） |
| 示意图作者画得比你能重绘得更清楚 | **原图**（准确优先） |

原图嵌入的硬要求：

- **注明编号**：画面角落标 `Fig. 3 · <论文短名>`，不整页搬运；
- **裁切干净**：去掉页码 / 页眉 / 大块白边；
- **纸面卡片**：论文图多为白底，暗色主题下放在浅底卡片上，不反色、
  不加滤镜；
- **仍要逐步揭示**：用 accent 高亮框 / 箭头 / 局部放大 / 逐块裁剪，
  由 `step` 驱动——**不允许静态贴图一屏到底**。
